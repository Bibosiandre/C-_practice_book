...
static bool AddProduct(int categoryId, string productName, decimal? price)
{
  using (Northwind db = new())
  {
    ...
    // пометить продукт как добавленный в отслеживании изменений
    db.Products.Add(p);

    // сохранять отслеживаемые изменения в базе данных
  ...
  string productNameStartsWith, decimal amount)
{
  using (Northwind db = new())
  {
    // получить первый продукт, имя которого начинается с имени
