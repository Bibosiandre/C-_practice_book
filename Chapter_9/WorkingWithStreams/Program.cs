...
static void WorkWithCompression(bool useBrotli = true)
{
  ...
  using (compressor)
  {
    using (XmlWriter xml = XmlWriter.Create(compressor))
    {
      ...
      // потому что при удалении XmlWriter он будет 
      // автоматически завершаем любые элементы любой глубины
    }
  } // also closes the underlying stream

  // также закрывает основной поток
  WriteLine("{0} contains {1:N0} bytes.",
  ...
  using (decompressor)
  {
    ...
    {
      while (reader.Read()) // прочитать следующий узел XML
      {
        // проверяем, находимся ли мы на узле элемента с именем позывной
        if ((reader.NodeType == XmlNodeType.Element)
          && (reader.Name == "callsign"))
        {
          reader.Read(); // перейти к тексту внутри элемента
          WriteLine($"{reader.Value}"); // прочитать его значение
        }
      }
    }
  }
